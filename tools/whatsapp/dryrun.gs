...
var TEXT_E = {
  W1: { de: 'Hey {name} 👋 wir haben gesehen, dass die letzte Zahlung bei deinem Abo leider nicht durchgegangen ist. Kannst du bitte kurz deine Zahlungsdaten und die Deckung deines Kontos prüfen, damit wir es in den nächsten Tagen erneut abbuchen können? Eine neue Karte kannst du hier hinterlegen: {card_link} (oder in der IMPACT App unter Billing Info, Payment Methods). Wenn du Hilfe brauchst, sag kurz Bescheid 🙏 Danke dir!', // Ruben 22.09. (go): Waseem's additions = card link + app path in W1, amount + due date + card link in W2, fee sentence in W4 (W4_FEE); W2 moved to day 7
        en: "Hey {name} 👋 We noticed that the last payment for your membership didn't go through. Could you please check your payment details and make sure your account has sufficient funds, so we can retry the charge in the next few days? You can save a new card here: {card_link} (or in the IMPACT app under Billing Info, Payment Methods). If you need any help, just let us know 🙏 Thanks so much!" },
  W2: { de: 'Hey {name}, wir konnten die offene Zahlung von CHF {amount}, fällig seit dem {due_date}, leider immer noch nicht abbuchen. Bitte prüfe heute kurz deine Zahlungsdaten oder die Deckung deines Kontos, wir versuchen die Abbuchung dann nochmals. Eine neue Karte kannst du hier hinterlegen: {card_link} Danke dir!',
        en: "Hey {name}, unfortunately we still couldn't collect the outstanding payment of CHF {amount}, due since {due_date}. Please check your payment details and the funds on your account today, and we will retry the charge. You can save a new card here: {card_link} Thanks!" },
...
var W4_FEE = { on: false, de: ' Dabei kommt eine Inkassogebühr von CHF 100 dazu.', en: ' This adds a debt collection fee of CHF 100.' }; // Ruben 22.09.: fee sentence in W4 only once the new GTC (14.09.2026, point 7) apply to the members: notification + 30 days without objection, i.e. from about 23.10.2026, and never for members who objected (switch stays off until Ruben says so)
...
function waDryRunHourly() {
  ...
  // W1 three days after the first open invoice, W2 day 7 (Ruben 22.09., was day 6), W3 day 14 with the pay link of the ORIGINAL open invoice, W4 as soon
...
function firstOf(n) { return capName(String(n || '').split(' ')[0].replace(/[&+]/g, ' & ').replace(/\s+/g, ' ').trim()); }
function capName(n) { return String(n || '').replace(/[^\s\-&]+/g, function (w) { if (w.length > 1 && w === w.toUpperCase() && w !== w.toLowerCase()) w = w.toLowerCase(); return w.charAt(0).toUpperCase() + w.slice(1); }); } // Ruben 25.09.: "emmanuel" -> "Emmanuel", "jean-luc" -> "Jean-Luc", "EMMA" -> "Emma"; mixed case (McKay) stays
...
var PERSON_BLOCK = { D: true }; // Ruben 25.09.: the follow-up after the trial is also skipped as soon as the person wrote to us after the trial started (a reply to the class reminder before the trial does not count)
...
var SEND = { on: true, flows: { A: true, B: true, C: true, D: true, X: false, E: false, R: true }, conn: { 'Zürich': 'zh', 'Support (Waseem)': 'ws' }, since: '2026-09-23', since_flow: { A: '2026-09-25 18:10', C: '2026-09-25 19:20', D: '2026-09-25 19:20', R: '2026-09-25 19:20' }, max_per_run: 40, max_age_h: 36 }; // Flow A Zürich LIVE since 25.09.2026 18:10 (Ruben: "let's go"); C, D, R Zürich LIVE since 25.09.2026 19:20 (Ruben: "Go C, D und R"); max_age_h: a row detected longer ago than this is never sent (expired), so a connection that goes live later (Winterthur) does not release stale rows. // since_flow: per flow the local date-time ('2026-09-26 10:00') from which rows count as real; rows detected before are dry-run history (set when a flow goes live, so no backlog is sent). Flow B Zürich LIVE since 25.09.2026 (Ruben: "Go Flow B"); ws = Waseem's number (connected 25.09.), used by Flow E once SEND.flows.E is on; // the real sender (23.09.2026): master switch + one switch per flow, only rows detected since "since"; connections: Zürich = Abdi (zh); Winterthur / Waseem follow when their numbers are connected. Everything stays off until Ruben gives the go per flow
...
function processOutbox(ss, dry, now, stillDue) { // 1. update the delivery status of sent messages from "WA Events" (both outboxes); 2. if SEND.on: send every Dry run row that is due now, enabled, in the window and still valid; returns a note for the status line
  ...
  var chatAt = {}, chatTxt = {}, autoAt = {}, inAt = {}; // phone -> last message a coach / Waseem wrote from the app (any connected number); pnid|phone -> coach texts (language); phone -> last automatic send; phone -> last message of the person
...
function waDocWaseem() { // one-off (Ruben 22.09., go): Google Doc "WhatsApp Messages IMPACT", Flow E table: W1 / W2 / W4 texts with Waseem's additions, "When" cells back to Ruben's timing (W2 now day 7), Waseem's two proposal columns removed (decided), e-mail signature without "IMPACT Support Team", trigger paragraph and version updated
  var body = DocumentApp.openById('1EwWEOWUgU1YpO9Ee18DpJTuxuIqcQAmglqPVm65K0VY').getBody(), t = null;
  body.getTables().forEach(function (x) { if (x.getNumRows() > 1 && x.getRow(1).getCell(0).getText().trim() === 'W1' && x.getRow(0).getNumCells() >= 4) t = x; });
  if (!t) throw new Error('W table not found');
  var when = { W1: 'Day 3 after the first failed invoice', W2: 'Day 7, still unpaid', W3: 'Day 14, still unpaid, Stripe has stopped retrying', W4: 'As soon as a second invoice is open; 7-day deadline. The fee sentence goes out only once the new GTC apply (from about 23.10.2026) and never to members who objected', W5: 'One day after a second invoice is paid while an older one is still open' };
  var texts = { W1: [TEXT_E.W1.de, TEXT_E.W1.en], W2: [TEXT_E.W2.de, TEXT_E.W2.en], W4: [fill(TEXT_E.W4.de, { fee: W4_FEE.de }), fill(TEXT_E.W4.en, { fee: W4_FEE.en })] };
  var n = 0, removed = 0;
  for (var r = 1; r < t.getNumRows(); r++) { var row = t.getRow(r), id = row.getCell(0).getText().trim(); if (when[id]) { row.getCell(1).setText(when[id]); n++; } if (texts[id]) { row.getCell(2).setText(texts[id][0]); row.getCell(3).setText(texts[id][1]); n++; } }
  for (var r2 = 0; r2 < t.getNumRows(); r2++) { var rw = t.getRow(r2); while (rw.getNumCells() > 4) { rw.removeCell(rw.getNumCells() - 1); removed++; } }
  var reps = [['W2 on day 6', 'W2 on day 7'], ['Version 10 Sep 2026', 'Version 22 Sep 2026']]; reps.forEach(function (p) { if (body.findText(p[0])) body.replaceText(p[0], p[1]); });
  var sig = 0; body.getParagraphs().forEach(function (p) { if (p.getText().trim() === 'IMPACT Support Team') { p.removeFromParent(); sig++; } });
...
function waTrialTags(dry) { // own trigger every 15 minutes (installTagTrigger): ONE bookings report for both studios, split by the Location column (25.09.: per-studio calls got Zurich's cached report for Winterthur); stage from the client search. dry = preview: logs who would get a tag, writes nothing
  ...
  ['Zurich', 'Winterthur'].forEach(function (loc) {
    ...
    todo.forEach(function (u) {
      ...
      if (attended[u]) { skipped++; if (!dry) { log.done[u + ':' + tag] = true; add.push([dayStart(now), fmtT(now), u, c.name || '', tag, 'skipped', 'already had a trial on ' + euD(attended[u]) + ' (booking ' + uids[u] + ')']); } else would.push(loc + ' | ' + (c.name || u) + ' | NOT TAGGED: already had a trial on ' + attended[u]); return; } // Ruben 25.09.: a second session is no trial, no welcome mail
...
var UNDELIV = { on: true, to: { 'Zürich': 'abdi@impact-martialarts.com' }, from: '2026-09-25', weekdays: [1, 2, 3, 4, 5], hour: 12 }; // Ruben 25.09.: daily mail to the studio manager with the automatic WhatsApp messages Meta could not deliver; Mon-Fri at 12:00, each mail covers every day since the last mail (Monday = Friday + Saturday). on since 25.09.2026 22:35 (Ruben: "passt")
