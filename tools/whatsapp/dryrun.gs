...
var TEXT_E = {
  W1: { de: 'Hey {name} 👋 wir haben gesehen, dass die letzte Zahlung bei deinem Abo leider nicht durchgegangen ist. Kannst du bitte kurz deine Zahlungsdaten prüfen/aktualisieren und uns Bescheid geben, sobald das erledigt ist, damit wir es erneut abbuchen können? Wenn du Hilfe brauchst, sag kurz Bescheid 🙏 Danke dir!',
        en: "Hey {name} 👋 We noticed that the last payment for your membership didn't go through. Could you please take a moment to check or update your payment details and let us know once it's done, so we can retry the charge? If you need any help, just let us know – we're happy to assist! 🙏 Thanks so much!" },
  W2: { de: 'Hey {name}, wir konnten die offene Zahlung leider immer noch nicht abbuchen. Bitte aktualisiere heute kurz deine Zahlungsdaten und gib mir Bescheid, dann buchen wir gleich nochmals ab. Danke dir!',
        en: 'Hey {name}, unfortunately we still could not collect the outstanding payment. Please update your payment details today and let me know, then we will retry the charge right away. Thanks!' },
  W3: { de: 'Hey {name}, wir melden uns nochmals wegen der weiterhin offenen Zahlung (seit über 10 Tagen). Bitte bring das heute in Ordnung (Zahlungsdaten aktualisieren), damit dein Abo sauber weiterläuft. Falls es gerade schwierig ist: melde dich kurz, dann finden wir eine Lösung.',
