...
function wrMerge(o, x) { o.dup += x.dup; ['leads', 'att', 'calls', 'placed', 't', 'ns', 'sold', 'csold'].forEach(function (f) { o[f].Zurich += x[f].Zurich; o[f].Winterthur += x[f].Winterthur; }); ['kanal', 'inter', 'tk'].forEach(function (f) { ['Zurich', 'Winterthur'].forEach(function (l) { Object.keys(x[f][l]).forEach(function (k) { o[f][l][k] = (o[f][l][k] || 0) + x[f][l][k]; }); }); }); return o; }
...
function wrBlank() { var pl = function (v) { return { Zurich: v(), Winterthur: v() }; }, z = function () { return 0; }, o = function () { return {}; }; return { dup: 0, leads: pl(z), kanal: pl(o), inter: pl(o), att: pl(z), calls: pl(z), placed: pl(z), t: pl(z), ns: pl(z), sold: pl(z), csold: pl(z), tk: pl(o) }; }
...
function wrCollect(ss, team) {
  ...
  ['Zurich', 'Winterthur'].forEach(function (loc) {
    ...
    ts.getRange(TR_ROW0, TR_P0, n, TR_NCOL).getValues().forEach(function (r) {
      if (!r[CI.uid]) return; var d = dOfCell(r[CI.date]); if (!d || !trIsTrial(r)) return;
      var kn = String(r[CI.kanal] || ''); both(d).forEach(function (o) { o.tk[loc][kn] = (o.tk[loc][kn] || 0) + 1; }); // Probetrainings je Kanal (CPT je Plattform im Tab Werbekosten)
      if (dOfCell(r[CI.start])) both(d).forEach(function (o) { o.csold[loc] += 1; });
    }); // Kohorte: Abo bis heute gestartet (Vertragsstart), gleiche Regel wie der Monatsreport
  });
  return out;
}
...
var WK_NOTE = 'Werbekosten und Wirkung je Standort, gleicher Aufbau wie der Monatsabschluss: Wochen vor ihrem Monat, Monate von links nach rechts, Jahr rechts. Zeilen mit + zeigen die Plattformen. Der Abschnitt Kampagnen zeigt je Kampagne Kosten, Website-Leads und Kosten pro Lead untereinander. Definitionen als Notiz an der Zeile und im Tab Methodik.';
...
function wkCampAgg(ss) {
  var st = stGet(ss), share = Number(st['Werbekosten-Split ohne Standort']); if (!(share >= 0 && share <= 1)) share = 0.5;
  var out = { list: {}, day: {}, month: {} }, recentFrom = fmtD(addD(new Date(), -60));
  wkRead(ss).forEach(function (x) {
    if (!x.d || !x.camp) return;
    var key = x.plat + '|' + wkCampKey(x.camp), mk = x.d.slice(0, 7);
    var L = out.list[key] = out.list[key] || { plat: x.plat, camp: x.camp, key: key, loc: x.loc, tot: 0, cid: '', recent: false };
    L.tot += x.cost; if (x.cid) L.cid = x.cid; if (x.d >= recentFrom && x.cost > 0) L.recent = true;
    var parts = x.loc === 'Zurich' ? [['Zurich', 1]] : x.loc === 'Winterthur' ? [['Winterthur', 1]] : [['Zurich', share], ['Winterthur', 1 - share]];
    var D = out.day[key] = out.day[key] || {}, M = out.month[key] = out.month[key] || {};
    var dd = D[x.d] = D[x.d] || { Zurich: 0, Winterthur: 0 }, mm = M[mk] = M[mk] || { Zurich: 0, Winterthur: 0 };
    parts.forEach(function (pt) { dd[pt[0]] += x.cost * pt[1]; mm[pt[0]] += x.cost * pt[1]; });
  });
  return out;
}
// Website-Leads je Kampagne, Tag und Standort (Daten: Datum, Standort, Interesse, Zaehlt, Kanal; Leads: Seite, utm_campaign; gleiche
...
function wkLeadsByCampaignDaily(ss, camp) {
  ...
  var byId = {}, cands = {};
  // Kampagnen-Nummer -> Kampagne; tragen zwei Namen dieselbe Nummer (umbenannt), gewinnt die mit aktuellen Kosten, sonst die teurere
  Object.keys(camp.list).forEach(function (k) { var L = camp.list[k]; if (L.cid) { var idk = L.plat + '|' + L.cid, ex = byId[idk] ? camp.list[byId[idk]] : null; if (!ex || (L.recent && !ex.recent) || (L.recent === ex.recent && L.tot > ex.tot)) byId[idk] = k; } if (L.recent) (cands[L.plat + '|' + L.loc] = cands[L.plat + '|' + L.loc] || []).push(k); });
  var qp = function (url, name) { var m = String(url || '').match(new RegExp('[?&]' + name + '=([^&#]*)')); if (!m) return ''; try { return decodeURIComponent(m[1].replace(/\+/g, ' ')); } catch (e) { return m[1]; } };
  for (var i = 0; i < n; i++) {
    var d = dOfCell(dv[i][0]), plat = String(dv[i][9] || ''), locDE = String(dv[i][3] || ''), L = locDE === 'Zürich' ? 'Zurich' : locDE === 'Winterthur' ? 'Winterthur' : null;
    if (!d || d < WK_ATTR_FROM || !L || Number(dv[i][6]) !== 1 || WK_PLATFORMS.indexOf(plat) < 0) continue;
    var key = '', page = String(lv[i][13] || ''), utm = wkCampKey(lv[i][23]) || wkCampKey(qp(page, 'utm_campaign'));
    // 1. Kampagnen-Nummer aus dem Anzeigen-Link (Meta utm_id, Google gad_campaignid): ueberlebt Umbenennungen (Lehre 15.09.: Links mit altem Namen "PM" statt "MK")
    var cid = plat === 'Meta Ads' ? qp(page, 'utm_id') : plat === 'Google Ads' ? qp(page, 'gad_campaignid') : '';
    if (cid && byId[plat + '|' + cid]) key = byId[plat + '|' + cid];
    // 2. Kampagnenname aus dem Link (Spalte utm_campaign, sonst aus der gespeicherten Seiten-URL)
    if (!key && utm && camp.list[plat + '|' + utm]) key = plat + '|' + utm;
    // 3. Google ohne Nummer und Namen: Standort plus Little Ninjas/Erwachsene, wenn genau eine aktive Kampagne passt
    if (!key && plat === 'Google Ads') {
      { var kids = /little ninjas|kids/i.test(String(dv[i][4] || '') + ' ' + String(lv[i][7] || '')); var cs = (cands[plat + '|' + L] || []).concat(cands[plat + '|Beide'] || []).filter(function (k) { return /ninja|kids/i.test(k) === kids; }); if (cs.length === 1) key = cs[0]; }
    }
    if (!key) continue;
...
function runWerbekostenBuild() {
  ScriptApp.getProjectTriggers().forEach(function (t) { if (t.getHandlerFunction() === 'runWerbekostenBuild') ScriptApp.deleteTrigger(t); });
  try { buildWerbekosten(SpreadsheetApp.openById(SHEET_ID)); } catch (e) { mailOnce('wkbuild', '[Sheet] Werbekosten-Bau FEHLGESCHLAGEN', String(e && e.stack ? e.stack : e)); }
}
// Werbekosten-Tab (Ruben 08.09.2026 nachmittags): gleicher Aufbau wie der Monatsabschluss (Spalten aus maContext), drei Bloecke
...
function buildWerbekostenCore(ss, ctx) {
  ...
  sh.getRange('A2').setValue('Methodik').setFontColor('#999999'); sh.getRange('B2').setValue(WK_NOTE).setFontColor('#666666').setWrap(true); sh.getRange('B2:N2').merge(); sh.setRowHeight(2, 44);
  var r = 4, blocks = [];
  ['Zurich', 'Winterthur', 'Gesamt'].forEach(function (loc) {
    ...
    var monthO = function (kk) { return wr.month[kk] || wrBlank(); }, weekO = function (c) { return ctx.weekOf(c.k, c.mk); };
    var daysOf = function (c) { var out = []; for (var i = 0; i < 7; i++) { var d = addDs(c.k, i); if (d.slice(0, 7) === c.mk) out.push(d); } return out; };
    ...
    var leadsM = function (kk) { return kk >= logM ? wrSum(monthO(kk), 'leads') : vOf(kk, 'leads_web'); }, leadsW = function (c) { return wrSum(weekO(c), 'leads'); };
    var kanM = function (kk, pn) { return kk >= logM ? wrKan(monthO(kk), 'kanal', pn) : ''; }, kanW = function (c, pn) { return wrKan(weekO(c), 'kanal', pn); };
    var trialsM = function (kk) { return vOf(kk, 'trial_attended'); }, trialsW = function (c) { return wrSum(weekO(c), 't'); };
    var tkM = function (kk, pn) { return kk >= MA_TEAM_FROM ? wrKan(monthO(kk), 'tk', pn) : ''; }, tkW = function (c, pn) { return wrKan(weekO(c), 'tk', pn); };
    var salesM = function (kk) { return vOf(kk, 'sales_signed'); }, salesW = function (c) { return ctx.daySumFor(wrLocs, c.k, c.mk, 'signed_d:'); };
    var skM = function (kk, pn) { return vOf(kk, 'sales_kanal:' + pn); };
    ...
    var M = function (fnM, fnW) { return function (c) { return c.w ? (fnW ? fnW(c) : '') : fnM(c.k); }; };
    // Jahr: Quote aus den Summen, nur Monate mit Zaehler UND Nenner; 'last' = letzter Monat mit Wert
    var yRatio = function (nf, df) { return function (mks) { var n = 0, d = 0, any = false; mks.forEach(function (kk) { var a = nf(kk), b = df(kk); if (a === '' || b === '') return; any = true; n += num(a); d += num(b); }); return any && d ? n / d : ''; }; };
    var yLast = function (f) { return function (mks) { var v = ''; mks.forEach(function (kk) { var x = f(kk); if (x !== '') v = x; }); return v; }; };
    var yCacAll = function (mks) { var m = 0, sg = 0, any = false; mks.forEach(function (kk) { var a = mediaM(kk), b = salesM(kk); if (a === '' || b === '') return; any = true; m += num(a) + num(agencyOf(kk)); sg += num(b); }); return any && sg ? m / sg : ''; };
    ...
    // Kosten
    writeRow('wk_media', 'Werbekosten Media (CHF)', M(function (kk) { return mediaM(kk); }, function (c) { return mediaW(c); }), '#,##0', { bold: true, weekly: true, year: 'sum' });
    WK_PLATFORMS.forEach(function (pn) {
      writeRow('wk:' + pn, '   ' + pn + ' (CHF)', M(function (kk) { return mediaM(kk, pn); }, function (c) { return mediaW(c, pn); }), '#,##0', { detail: 1, weekly: true, year: 'sum', note: 'Media-Kosten ' + pn + ', Standort aus dem Kampagnennamen. Je Kampagne: Abschnitt Kampagnen weiter unten.' });
    });
    ...
    writeRow('leads_web', 'Website-Leads (aus dem Log)', M(leadsM, leadsW), '0', { bold: true, weekly: true, year: 'sum', note: 'Website-Anfragen aus dem Log (ohne Dubletten und Tests), vor September 2026 die Monatszahl aus der Historie. Plus: je Kanal (Klick-ID, UTM oder Referrer der Anfrage, erst ab 02.09.2026). Je Kampagne: Abschnitt Kampagnen weiter unten. Anrufe und Walk-ins nach einer Anzeige stehen unter organisch/direkt.' });
    WK_PLATFORMS.forEach(function (pn) {
      writeRow('leads:' + pn, '   ' + pn, M(function (kk) { return kanM(kk, pn); }, function (c) { return kanW(c, pn); }), '0', { detail: 1, weekly: true, year: 'sum', note: 'Website-Leads, deren Anfrage über ' + pn + ' kam (letzter Klick). Je Kampagne: Abschnitt Kampagnen weiter unten.' });
    });
    ...
    WK_PLATFORMS.forEach(function (pn) {
      writeRow('cpl:' + pn, '   CPL ' + pn + ' (CHF)', M(function (kk) { return ratio(mediaM(kk, pn), kanM(kk, pn)); }, function (c) { return ratio(mediaW(c, pn), kanW(c, pn)); }), '#,##0', { detail: 1, weekly: true, year: yRatio(function (kk) { return mediaM(kk, pn); }, function (kk) { return kanM(kk, pn); }), note: 'Media-Kosten ' + pn + ' geteilt durch Website-Leads mit Klick-ID oder UTM von ' + pn + '.' });
    });
    // Kampagnen (Ruben 15.09.): je Kampagne Kosten, Website-Leads und Kosten pro Lead direkt untereinander statt verteilt auf drei Abschnitte
    writeRow('camp_head', 'Kampagnen: Kosten, Website-Leads, Kosten pro Lead', function () { return ''; }, null, { bold: true, note: 'Je Kampagne drei Zeilen: Media-Kosten (CHF), Website-Leads aus ihren Anzeigen-Links, Kosten pro Lead. Reihenfolge nach Kosten, nur Kampagnen mit Kosten in den gezeigten Monaten. ' + WK_LEADS_NOTE });
    WK_PLATFORMS.forEach(function (pn) {
      var ks = campsOf(pn); if (!ks.length) return;
      writeRow('camp:' + pn, '   ' + pn, function () { return ''; }, null, { detail: 1 });
      ks.forEach(function (k) {
        var nm = camp.list[k].camp + (camp.list[k].loc === 'Beide' ? ' (beide Standorte, anteilig)' : '');
        writeRow('wkc', '      Kosten (CHF) · ' + nm, M(function (kk) { return campM(k, kk); }, function (c) { return campW(k, c); }), '#,##0', { detail: 2, weekly: true, year: 'sum', note: 'Media-Kosten der Kampagne ' + nm + ' (' + pn + ').' });
        writeRow('leadsc', '      Leads · ' + nm, M(function (kk) { return lcM(k, kk); }, function (c) { return lcW(k, c); }), '0', { detail: 2, weekly: true, year: 'sum', note: 'Website-Leads aus den Anzeigen-Links der Kampagne ' + nm + ' (Kampagnen-Nummer oder Name im Link), seit 01.09.2026.' });
        writeRow('cplc', '      CHF pro Lead · ' + nm, M(function (kk) { return ratio(campM(k, kk), lcM(k, kk)); }, function (c) { return ratio(campW(k, c), lcW(k, c)); }), '#,##0', { detail: 2, weekly: true, year: yRatio(function (kk) { return campM(k, kk); }, function (kk) { return lcM(k, kk); }), note: WK_LEADS_NOTE });
      });
    });
    writeRow('cpt', 'Kosten pro Probetraining (CHF)', M(function (kk) { return ratio(mediaM(kk), trialsM(kk)); }, function (c) { return ratio(mediaW(c), trialsW(c)); }), '#,##0', { bold: true, weekly: true, year: yRatio(mediaM, trialsM) });
    WK_PLATFORMS.forEach(function (pn) { writeRow('cpt:' + pn, '   CPT ' + pn + ' (CHF)', M(function (kk) { return ratio(mediaM(kk, pn), tkM(kk, pn)); }, function (c) { return ratio(mediaW(c, pn), tkW(c, pn)); }), '#,##0', { detail: 1, weekly: true, year: yRatio(function (kk) { return mediaM(kk, pn); }, function (kk) { return tkM(kk, pn); }), note: 'Media-Kosten ' + pn + ' geteilt durch durchgeführte Probetrainings, deren Website-Anfrage über ' + pn + ' kam (Kanal im Team-Sheet), ab September 2026.' }); });
    // Kosten je Verkauf
    writeRow('cac', 'CAC Media (CHF je Verkauf)', M(function (kk) { return ratio(mediaM(kk), salesM(kk)); }, function (c) { return ratio(mediaW(c), salesW(c)); }), '#,##0', { bold: true, weekly: true, year: yRatio(mediaM, salesM) });
    WK_PLATFORMS.forEach(function (pn) {
      writeRow('sk:' + pn, '   Verkäufe aus ' + pn + '-Leads', M(function (kk) { return skM(kk, pn); }), '0', { detail: 1, year: 'sum', note: 'Verkäufe des Monats, deren Website-Anfrage über ' + pn + ' kam (Klick-ID oder UTM, letzter Klick).' });
      writeRow('cac:' + pn, '   CAC ' + pn + ' (CHF je Verkauf)', M(function (kk) { return ratio(mediaM(kk, pn), skM(kk, pn)); }), '#,##0', { detail: 1, year: yRatio(function (kk) { return mediaM(kk, pn); }, function (kk) { return skM(kk, pn); }) });
    });
    writeRow('cac_all', 'CAC inkl. Agentur (CHF je Verkauf)', M(cacAllM), '#,##0', { bold: true, year: yCacAll });
...
function runWerbekosten() {
  ...
  buildMonatsabschluss(ss); // baut den Werbekosten-Tab mit (gleiche Daten, gleicher Aufbau)
...
var MA_NOTES = {
  ...
  wk_total: 'Media-Kosten plus Agenturkosten des Monats.',
  cpt: 'Media-Kosten geteilt durch durchgeführte Probetrainings (Erstbesucher mit Check-in). Plus: je Plattform nach Kanal der Website-Anfrage.',
  paid_share: 'Verkäufe, deren Website-Anfrage über Google Ads, Meta Ads oder TikTok Ads kam, geteilt durch alle Verkäufe des Monats (Klick-ID oder UTM des Leads, letzter Klick).',
...
// Daten und Spalten fuer Monatsabschluss UND Werbekosten (gleicher Aufbau, Ruben 08.09.2026): Werte aus MonatsHistorie, Log, Team-Sheet,
// Werbedaten, Bank; Spalten = Monate ab MA_FROM, Kalenderwochen vor ihrem Monat, Jahresspalten
function maContext(ss) {
...
function buildMonatsabschlussCore(ss) {
  ...
  ['Zurich', 'Winterthur', 'Gesamt'].forEach(function (loc) {
    ...
    put('ltv', 'LTV netto (CHF, Prognose)', V('ltv_forecast'), '#,##0', B); // Werbekosten, CPL, CAC, LTV : CAC, Payback stehen im Tab Werbekosten (Ruben 08.09.: keine Redundanz)
...
var TR_T = {
  de: {
    ...
    legend: 'Farben der Zeile: satt grün = Paketstart da (Verkauf) · hellgrün = Vertrag unterschrieben, Paketstart fehlt noch · orange = seit 7 Tagen oder länger unterschrieben, kein Paketstart, Stage „Signed but no payment“ · rot = in „Prüfen“ steht ein Hinweis · gelb = Kein Check-in (prüfen), Wiederholer, Rückkehrer, Event · graue Schrift = No-Show, Storniert, Gebucht (kommend) · grau durchgestrichen = Non-Client, zählt nicht.',
    legendNote: 'Die Farbe gilt immer für die ganze Zeile einer Person.\n\nSatt grün (weisse Schrift): Paketstart ist eingetragen, das erste Abo-Paket ist aktiviert. Das ist der Verkauf.\n\nHellgrün: Vertrag unterschrieben (Waiver), Paketstart noch nicht oder erst geplant.\n\nOrange: Vertragsunterschrift seit 7 Tagen oder länger, kein Paketstart, Stage „Signed but no payment“. Unterschrieben, aber ohne Zahlungsdaten: nachfassen. Ein geplanter Paketstart steht als Datum in der Spalte Paketstart und wird deshalb nicht orange.\n\nRot: In der Spalte „Prüfen“ steht ein Hinweis. Fakt (Buchung, Check-in, Vertrag) und Lifecycle-Stage passen nicht zusammen, bitte in exercise.com die Stage setzen. Beim nächsten Lauf verschwindet die Farbe.\n\nGelb: Kein Check-in (prüfen), Wiederholer (prüfen), Rückkehrer (Ex-Mitglied) oder Event (kein Trial).\n\nGraue Schrift: No-Show, Storniert oder Gebucht (kommend).\n\nGrau durchgestrichen: Stage „Non-Client“ (Assistant Coach, Friends & Family), zählt nirgends mit.\n\nSpaltenerklärungen: Notiz auf der Überschrift.',
  ...
  en: {
    ...
    legend: 'Row colours: solid green = package start entered (sale) · light green = contract signed, package start still missing · orange = signed 7 days ago or longer, no package start, stage "Signed but no payment" · red = the "Check" column has a hint · yellow = No check-in (check), repeat visitor, returning ex-member, event · grey text = no-show, cancelled, booked (upcoming) · grey strikethrough = Non-Client, does not count.',
    legendNote: 'The colour always applies to the whole row of a person.\n\nSolid green (white text): a package start is entered, the first membership package is active. That is the sale.\n\nLight green: contract signed (waiver), package start still missing or only scheduled.\n\nOrange: contract signed 7 days ago or longer, no package start, stage "Signed but no payment". Signed, but no payment details: follow up. A scheduled package start shows as a date in the Package start column and therefore does not turn orange.\n\nRed: the "Check" column has a hint. Fact (booking, check-in, contract) and lifecycle stage do not match, please set the stage in exercise.com. The colour disappears with the next run.\n\nYellow: No check-in (check), repeat visitor (check), returning ex-member or event (no trial).\n\nGrey text: no-show, cancelled or booked (upcoming).\n\nGrey strikethrough: stage "Non-Client" (assistant coach, friends & family), does not count anywhere.\n\nColumn explanations: note on the header cell.',
...
function trCheck(r, today, T) {
  ...
  // Zahlung fehlt (Variante A, Ruben 09.09.): Vertragsunterschrift liegt TR_PAY_DAYS Tage oder laenger zurueck, es gibt keinen
  // Paketstart und die Stage steht auf "Signed but no payment". Ein geplanter Start steht als Datum in der Spalte Paketstart und
  // nimmt die Zeile damit automatisch aus der Regel. Die alte Regel (Paketstart + diese Stage) war unsinnig: mit Paketstart wird
  // die Stage in exercise.com automatisch "Client".
  var signed = dOfCell(r[CI.contract]);
  if (lc === LC_PAY_OPEN && signed && !contract) { var age = Math.round((new Date(today + 'T12:00:00') - new Date(signed + 'T12:00:00')) / 864e5); if (age >= TR_PAY_DAYS) return T.chk.pay.replace('{n}', age); }
  if (art === 'Trial') {
...
// Zeile 2: nur die Farb-Legende, sonst nichts (Ruben 09.09.). Die ausfuehrliche Erklaerung je Farbe haengt als Notiz an der Zelle,
// die Spaltenerklaerungen bleiben als Notiz auf den Ueberschriften, alle Regeln stehen im Tab Methodik des Analytics-Sheets.
function trA2(sh, T) {
  sh.getRange('A2').setValue(T.legend).setNote(T.legendNote).setFontColor('#666666').setWrap(true).setVerticalAlignment('top');
...
function trFormat(sh, n, loc) {
  ...
  if (n) {
    ...
    // Orange steht vor Rot: "unterschrieben, Zahlung fehlt" ist ein eigener Fall und soll nicht von der roten Pruefen-Farbe verdeckt werden
    rules.push(SpreadsheetApp.newConditionalFormatRule().whenFormulaSatisfied('=AND($' + cS + r0 + '<>"",$' + cV + r0 + '="",$' + cL + r0 + '="' + LC_PAY_OPEN + '",TODAY()-$' + cS + r0 + '>=' + TR_PAY_DAYS + ')').setBackground('#fdead1').setRanges([rng]).build()); // Variante A (Ruben 09.09.): Unterschrift alt, kein Paketstart
