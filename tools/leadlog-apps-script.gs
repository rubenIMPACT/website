...
function setupAnalyse() {
  ...
  var order = ['Leads-Analyse', 'Klassenanalyse', 'Kündigungsrisiko', 'Trainingsplan', 'Events', 'Trainingsplan-Analyse', 'Kündigungen', 'Leads Historie', 'Leads', 'Daten', 'PlanDaten', 'KlassenHistorie', 'KlassenHistorieDisziplin', 'RisikoHistorie'];
  var pos = 1;
...
var PLAN_COLS = ['ID','Website','Anmeldung','Rewards','Titel','Text','Bild-URL','Anmeldungen'];
...
function readEventsFrom(ps) {
  ...
  rows.forEach(function (r) {
    ...
    out.push({ id: String(g(r, 'ID') || ''), activity: String(r[0]), type: type, start: fmtDate(r[2]), end: fmtDate(r[3]),
      ...
      registration: g(r, 'Anmeldung') === true, rewards: g(r, 'Rewards') === true,
...
function updateSignupCount(ss, eventId) {
  try {
    ...
    planSheets().forEach(function (ps) { var last = ps.sh.getLastRow(); if (last < 2) return;
      var ids = ps.sh.getRange(2, ps.col['ID'], last - 1, 1).getValues();
      for (var i = 0; i < ids.length; i++) { if (String(ids[i][0]) === String(eventId)) { ps.sh.getRange(i + 2, ps.col['Anmeldungen']).setValue(n); return; } } });
  } catch (e) {}
}
