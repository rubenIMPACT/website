...
var PLAN_COLS = ['ID','Website','Anmeldung','Freunde','Rewards','Text','Bild-URL','Anmeldungen','CalId']; // Titel = Spalte Notes (Rubens Wunsch 03.09.)
...
function migrateOne(sh) {
  ...
  try {
    ...
    if (cI['Google event']) sh.getRange(1, cI['Google event']).setNote('GOOGLE-KALENDER\nHaken setzen = das Script legt innerhalb von 10 Minuten einen Kalendertermin an (Titel aus Notes, Zeit, Adresse, Text) und verschickt die Einladung an die IMPACT-Verteilerliste (aktuell im Test: nur ruben@). Haken entfernen = Termin wird wieder geloescht.');
...
var INVITE_LIST = ['ruben@impact-martialarts.com']; // Testphase (03.09.2026): nur Ruben. Spaeter Verteiler aller Mitarbeitenden eintragen.
  ...
  sheets.forEach(function (ps) {
    ...
    rows.forEach(function (r, i) {
      ...
      try {
        ...
        if (want && !ev) {
          ev = allDay ? cal.createAllDayEvent(title, st, { location: loc, description: desc, guests: INVITE_LIST.join(','), sendInvites: true })
                      : cal.createEvent(title, st, endOk ? en : new Date(st.getTime() + 90 * 60000), { location: loc, description: desc, guests: INVITE_LIST.join(','), sendInvites: true });
