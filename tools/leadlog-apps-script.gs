...
/* ===== Blog: Google Sheet "IMPACT Blog" -> /articles/ (18.09.2026, Ruben) =====
...
function blogSeed(sh) {
  var r = UrlFetchApp.fetch('https://raw.githubusercontent.com/rubenIMPACT/website/main/data/blog.json', { muteHttpExceptions: true });
  if (r.getResponseCode() !== 200) return 0;
  var posts = (JSON.parse(r.getContentText()).posts || []).slice().sort(function (a, b) { return a.date < b.date ? 1 : -1; });
...
function buildWerbekostenCore(ss, ctx) {
  ...
  ['Zurich', 'Winterthur', 'Gesamt'].forEach(function (loc) {
    ...
    // Aufbau (Ruben 16.09.2026): Standort-Zusammenfassung, dann jede Plattform genau EINMAL mit allem, was zu ihr gehoert
    // (Kosten, Leads, Kosten pro Lead, Probetrainings, Verkaeufe), direkt darunter ihre Kampagnen (Name als eigene Zeile,
    // dann Kosten / Leads / Kosten pro Lead, eine Gruppenstufe). Nichts mehr an mehreren Stellen.
    ...
    writeRow('wk_media', 'Media-Kosten (CHF)', M(function (kk) { return mediaM(kk); }, function (c) { return mediaW(c); }), '#,##0', { bold: true, weekly: true, year: 'sum' });
    writeRow('leads_web', 'Website-Leads (aus dem Log)', M(leadsM, leadsW), '0', { bold: true, weekly: true, year: 'sum', note: 'Website-Anfragen aus dem Log (ohne Dubletten und Tests), vor September 2026 die Monatszahl aus der Historie. Je Plattform und Kampagne: weiter unten.' });
    ...
    writeRow('trials', 'Probetrainings (durchgeführt)', M(trialsM, trialsW), '0', { weekly: true, year: 'sum', note: 'Erstbesucher mit Check-in (exercise.com), gleiche Zahl wie im Monatsabschluss.' });
    ...
    writeRow('sales', 'Verkäufe (Paket)', M(salesM, salesW), '0', { weekly: true, year: 'sum', note: 'Verkäufe des Monats (Vertragsunterschrift), gleiche Zahl wie im Monatsabschluss.' });
    ...
    writeRow('leads:org', '   Leads', M(orgM, orgW), '0', { weekly: true, year: 'sum' });
    writeRow('sales:org', '   Verkäufe', M(function (kk) { var sg = salesM(kk), pd = paidM(kk); return sg === '' || pd === '' ? '' : num(sg) - num(pd); }), '0', { year: 'sum', note: 'Verkäufe des Monats minus Verkäufe aus bezahlten Kanälen.' });
    ...
    Object.keys(rowMeta).forEach(function (rr) { var b = keep[locDE.split(' ')[0] + '|' + rowMeta[rr].parent + '|' + rowMeta[rr].label.trim()]; if (b) sh.getRange(Number(rr), 1, 1, cols.length + 1).setBackground(b); });
...
var MA_NOTES = {
  ...
  sales_signed: 'Verkäufe = Personen im exercise.com-Report "Sold Packages" des Monats (Ruben 09.09. und 10.09.): Abo-Pakete mit Zahlungsart Abo; Gratis-Pakete nur mit Tag "Rechnung"/"Invoice" am Kunden (Rechnungszahler); dazu das erste Personal-Training-Paket (8x/16x/32x) einer Person, die vorher nie ein Abo hatte. Keine Einzelsessions, keine Events, kein zweites oder weiteres PT-Paket. Eine Person zählt einmal. Kein Verkauf, wenn im selben Zug ein Abo oder PT-Paket endete (Paketwechsel, Wiedereinstieg: Ende zwischen 60 Tagen vor und 30 Tagen nach dem Verkaufstag). Verkaufstag = Aktivierung des Pakets. Geht so als "Neue Verkäufe" in den Finanzplan.',
  ...
  losses: 'Verluste = Personen im exercise.com-Report "Cancelled Subscriptions" mit Enddatum im Monat (Ruben 09.09. und 10.09., Variante A), mit denselben Ausschlüssen wie die Verkäufe: Abo-Pakete, dazu beendete PT-Pakete von Personen ohne Abo davor (Spiegel der PT-Verkäufe); keine Einzelsessions, keine Gratis-Abos, kein Paketwechsel oder Anschlusskauf (Converted, neues Abo oder neues PT-Paket zwischen 30 Tagen vor und 60 Tagen nach dem Ende). Eine Person zählt einmal. Nichtzahler (Debt collection) zählen nicht. Dieselbe Regel wie die Verlustquote im LTV. Geht so in den Finanzplan (Kündigungen).',
