...
- Sheet "IMPACT Blog" (ID 13PcwnjnrlX6kLLfrVWk3-sQTB9O7Z6aXD-_wBmUVQ7k, Drive folder "IMPACT Website" in Ruben's My Drive, own file because Google shares per file; sharing is up to Ruben). Tab "Posts", one row = one post: Website (tick = online) | Date | Title | Description (meta description) | Photo URL (Drive link or https) | Text | Slug (auto, never change) | Check (written by the script: OK / Missing: ...). Header notes explain every column.
...
- Apps Script (webapp version 34): `blogRead()` via `what=blog` (only ticked rows with Date/Title/Text; writes Slug and Check), `blogSetup()` formats an empty file (locale de_CH), `blogSeed()` filled the six old posts once (Script Property `blogSeeded`; checkbox cells count as content, so getLastRow is useless in that sheet).
