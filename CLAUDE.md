...
> Einstieg fuer Menschen: `README.md` und `docs/` (how-to, website, automations, apps-script, accounts). Diese Datei ist das chronologische Arbeitslog der KI-Sitzungen. Bei groesseren Aenderungen die passende Datei in `docs/` mitpflegen. Einmal-Skripte der Migration liegen in `tools/archive/` (nie wieder ausfuehren); `functions/api/schedule.js` wurde am 21.09.2026 geloescht (ungenutzt seit 17.09.).
...
## Regeln (verbindlich)
- Vor JEDEM Push: Playwright-Rendercheck (mobil 390px + Desktop 1280px) + HTML-Parse + Div-Balance (Anzahl `<div` == `</div>`) + JS-Syntax (`new Function` je `<script>`). Rot = kein Push. Bei JEDER Aenderung an Stundenplan-Daten (spdata, lndata, Kursseiten-#zeiten, training-plan SCHEDULE) zusaetzlich `python3 tools/schedule-check.py` (Zellen-Abgleich gegen data/plan-nominal.json = Google-Slides-Decks; Lehre 16.09.2026: vier 20:45-Open-Mats standen seit der Migration am 23.08. auf der Website, die es nie gab). Rot = kein Push.
...
- **Stundenplan-Wahrheit = Google-Slides-Decks (Ruben 16.09.2026)**: "Overall Schedule ZH 2.4" (1HH42ColN9v9Axln9vFy3kbnQvQD43TOyWq0mCPwpWu4, NUR Folie 1; Folie 2 = Zukunft) und "Overall Schedule WIN 2.2" (1o82fupJn7Y1PNecAVjGa5G8ziFEHG3QuwxTrtQfJR0o) sind der Nominalplan fuer spdata, lndata, Kursseiten-#zeiten UND das Training-Plan-Tool; exercise.com ueberschreibt NICHTS mehr: der Live-Merge ist seit 17.09.2026 auf Stundenplan- und Kids-Seiten ENTFERNT (Entscheid Ruben: die Folie reicht; functions/api/schedule.js bleibt ungenutzt liegen). Die Seiten bauen nur noch die Woche ab heute mit echten Daten aus spdata. Lesbar im Chrome-Praesentationsmodus (/present) mit Zoom; der Drive-Textexport verliert die Rasterposition. 16.09.: Little Ninjas 6-9 (Laszlo) 15:30 -> 15:40 (gilt in exercise.com ab 28.09., Website zeigt bis dahin per Live-Merge 15:30); WT Montag 18:50 BJJ (Gi) All Levels / 20:00 MMA All Levels (war vertauscht, auch im Tool). 20:45-Open-Mats stehen auf der Website, nicht auf der Folie - nicht entfernt, offen bei Ruben. GEBAUT 16.09.: Apps Script `spReadDecks()` liest beide Folien mit Koordinaten (what=plan), `spDaily()` 05:30 mailt Ruben Aenderungen (Trigger per what=sptrigger); Cloudflare `/api/plan-nominal` reicht durch (1h Cache, ?refresh=1); GitHub-Workflow `plan-sync.yml` taeglich 04:30 UTC: `tools/plan_from_api.py` -> `tools/build_plan.py` (alle 35 Stellen) -> `tools/schedule-check.py` -> Commit. Folie unlesbar (warnings) = kein Commit, Mail an Ruben. Handpflege der 35 Stellen ist damit abgeschafft: NUR die Folie aendern. STAND 16.09. ABENDS: Leser eingespielt (Webapp-Version neu, Slides-Freigabe durch Ruben erteilt), spDaily-Trigger 05:30 angelegt, Leser liest beide Folien IDENTISCH zur Abschrift (57/33 Zellen). Auf der ZH-Folie liegen unsichtbare Reste (Kuerzel RUB/JOR, alte Open-Mat-Karte unter Mo 12:00 Wrestling) - der Leser ignoriert sie (notes), Ruben kann sie loeschen. Abruf dauert 5-35 s; plan_from_api.py macht 3 Versuche. OFFEN: `.github/workflows/plan-sync.yml` liegt lokal, der Mac-PAT hat kein `workflow`-Recht -> Ruben ergaenzt das Recht oder legt die Datei im GitHub-Web an. Editor-Regel bestaetigt: get_page_text NIE auf der Editor-Seite (Token). Neue Webapp-Version noetig, sobald doGet oder von doGet gerufene Funktionen aendern.
- **Instagram-Feed LIVE (17.09.2026)**: `functions/api/instagram.js` liest via Graph API (System-User `sitefeed`, Token `IG_TOKEN` in Cloudflare, Ablauf nie, Rechte instagram_basic/pages_show_list/pages_read_engagement; App IMPACT Analytics mit Use Case 'Manage messaging & content on Instagram'; Freigabe der Token-Anfrage durch Sam = Vier-Augen-Prinzip von Meta). Seite Impact Martial Arts (112250768178801) -> IG-Konto 17841451669004279 (@impactmartialarts_ch). Liste 1h Cache, Bilder ueber ?img=<id> 7 Tage, `?debug=1` Diagnose. Die 6 Startseiten tauschen die 8 statischen Kacheln beim Laden aus; ohne Feed bleiben die statischen. META_ADS_TOKEN bleibt fuer die Werbekosten unangetastet.
- **Muay-Thai-Fotos (10.09.2026, Ruben)**: Block `/*mtphoto*/` im Head aller 4 Muay-Thai-Seiten (ZH/WT, DE/EN). `#warum` ("Lernen vom Weltmeister") zeigt `/assets/mt-sergei-belts.jpg` (Sergei mit fuenf Guerteln, Hochformat, 1600x2400, EXIF entfernt; Quelle Drive 1QLsVefxO5gKwejyFqq7SsV8DZD-kAa1-, geholt ueber /api/event-image): Desktop nur rechte Haelfte als Bildflaeche (`inset:0 0 0 46%`, linke Kante per mask-image weich), Mobile Position 15 % = Sergei rechts neben dem Text. `#sportart` ("Was ist Muay Thai?") hat jetzt das bisherige Sparring-Foto `/assets/mt-warum-right.jpg` mit demselben Fade: Desktop Text links (48 %) / Foto rechts, Desktop nutzt den Zuschnitt `/assets/mt-sparring-right.jpg` (47-86 % der Breite des Originals: nur Fenster + die beiden Frauen, ohne unscharfen Vordergrund links und ohne Wandkante rechts), `right 40%/cover`, Bildflaeche ab 40 %, Maske `transparent -> #000 55%`; der senkrechte Verlauf steht in FESTEN PIXELN (`#000 -> transparent 96px ... calc(100% - 56px) -> #000`) plus `padding-bottom:230px` und einen mehrstufigen Auslauf ueber die letzten 250 px, weil die Sektion auf Desktop nur ~460 px hoch ist und die alte Prozentangabe (80 %) die unteren 90 px Bild geschluckt hat (Rubens Einwand 10.09. 'unten zu frueh abgeschnitten') (Rubens Einwaende 10.09.: kein unscharfer Hintergrund hinter dem Text, Bild bis zum rechten Rand, langer Fade); Mobile Foto hinter dem Text, Position 62 %, Text 60 % breit, damit das Laecheln rechts frei bleibt. Drive-Bilder, die per Link nicht ladbar sind (Google-Anmeldeseite), ueber /api/event-image?id= holen.
...
## Werbekosten: Kampagnen-Zuordnung und Abschnitt Kampagnen (Ruben 15.09.2026, Commit 1632eeb)
- Befund: Meta Ads Manager meldete 28 Leads (CHF 17.84) fuer "UC // MK // Probetraining Zürich // cold", das Sheet 3 Leads (CHF 163). Ursachen: Meta zaehlt 7 Tage nach Klick / 1 Tag nach Ansicht inkl. Dubletten; Anzeigen-Links tragen teils den alten Namen "UC // PM // ..." (Kampagne umbenannt); vor 04.09. stand utm_campaign nicht in der Spalte; Link-in-Bio-Klicks (utm_content=link_in_bio, utm_medium=social) kommen mit fbclid und gelten im Kanal als "Meta Ads" (NICHT geaendert, offen).
- `wkLeadsByCampaignDaily`: Reihenfolge 1. Kampagnen-Nummer aus der Seiten-URL (Meta `utm_id`, Google `gad_campaignid`) gegen WerbekostenDaten Spalte J, 2. Name (Spalte utm_campaign, sonst `utm_campaign` aus der Seiten-URL), 3. Google-Fallback Standort + Kids. `WK_ATTR_FROM = '2026-09-01'`.
...
## Kanal-Regel Link in Bio + Kanal-Vergleich (Ruben 15.09.2026, Commit d008e21)
...
- Werbekosten-Tab je Plattform: statt monatsbezogener Probetrainings/CPT/Verkaeufe/CAC jetzt Kohorten-Zeilen (Spalte = Monat/Woche der Anfrage): "davon Probetraining (bis heute)", "davon Verkauf (bis heute)", "Quote Lead -> Verkauf", "Kosten pro Verkauf (CHF, Kohorte)". Quelle `wkCohorts(ss)`: Team-Sheet-Probetrainings (trIsTrial, nur Kanal *Ads) per E-Mail aus der exercise.com-Kundenliste (maCall Phase mc, Name -> E-Mail; Fallback Name) dem Website-Lead zugeordnet (Commit a4391f2; Name allein fand in WT nur 2 von 6), Lead-Datum = Kohortentag, Verkauf = Vertragsunterschrift (CI.contract) gesetzt. Log meldet zugeordnet/ohne Lead. Keys coh_t:/coh_s:/coh_q:/coh_cac:<pn>. tk:/cpt:/sk:/cac: je Plattform entfallen (tkM/skM bleiben fuer paidM).
...
## Nachtpause + Wochen-Zahlungen im Monatsabschluss (Ruben 17.09.2026, Commit d878308)
- `nightPause()` = Stunde 1-5 (Europe/Zurich): runProbetrainingsHourly (vorher 09-22!) und syncCalendar (10 Min) kehren sofort zurueck. Letzter Lauf 00:xx, erster 06:xx. Tagesjobs verschoben: runMonatsabschlussDaily 06:20 (war 04:30), spDaily 06:50 (war 05:30, jetzt in installTrialTriggers), runLTVMonthly 1. des Monats 07:00 (war 05:00), runWerbekostenDaily bleibt 06:30. TR_TRIG_VER 'np1' -> Trigger werden beim naechsten Stundenlauf neu angelegt.
- Zahlungen je Woche und laufender Monat: klassen.js ltv kind "sums" liefert je Tag/Standort zusaetzlich a[8] = Abo brutto; Tab ZahlungenTag hat Spalte "Abo brutto"; `ltvRefreshSums(ss, mk)` ersetzt die Zeilen des Monats und holt neu (in runMonatsabschluss, also 06:20 taeglich und im Nachlauf). MonatsHistorie-Tageskeys `cash_d:<Datum>` (Betrag - Refund) und `abo_d:<Datum>`; `cv_active_live` = subs_total - scheduled_subs (Stand des Laufs).
- Writer V(): cash_paid/abo_gross/one_gross Woche = daySum der Tageskeys, Monat = LTV-Wert, sonst monthDays(); cv_active Monat = cv_active oder cv_active_live, Woche = cv_active_live des Monats. Zeilen 35-41 (cash_paid, abo, one, cash_vat, cash_net, cv_abo_gross, cv_abo_net) jetzt weekly. Zeilen 28/34 laufender Monat = Stand heute.
- Laufzeit-Messung nach dem Umbau steht noch aus (Ruben: "Miss danach, wie lange es dauert").
- 17.09.2026 (Commit dd3226b, Ruben): syncCalendar alle 3 h statt alle 10 Min, importKlassenanalyse taeglich 06:10 statt stuendlich (der stuendliche Trigger war von Hand angelegt); beide jetzt in installTrialTriggers verwaltet, TR_TRIG_VER np2. Laufzeit-Messung 17.09. 04-09 Uhr: syncCalendar 32x/450 s, Werbekosten-Bau 3x/308 s, Klassenanalyse 5x/294 s, MA daily 201 s -> hochgerechnet 2-2.5 h/Tag von 6 h Budget (vor der Ausduennung).

## Randwochen in beiden Monaten + zwei neue Monatsabschluss-Diagramme (Ruben 17.09.2026, Commit e60815d)
- `weeksOf(mk)`: jede Woche mit Tagen im Monat steht bei diesem Monat (vorher: Monat des Donnerstags, dadurch fehlten Randtage wie 28.-30.09. in jeder Woche). Randwochen erscheinen zweimal, je mit ihren Tagen; Kopf "KW 40 (Teil)". Wochenwerte zaehlen nur Tage des Monats (daySumFor/wrWeekOf), Summe der Wochen = Monat. In den Wochendiagrammen werden Montage dedupliziert (wkeys unique).
- Diagramme Monatsabschluss je Block jetzt 3 Baender: zuerst "Umsatz pro Monat (CHF brutto)" (Zahlungen exercise.com = abo+one bzw. Tagessummen im laufenden Monat, Bankeingang = Summe bankOf, davon Abo, davon Einzelverkaeufe) und "Verkaeufe, Verluste und Nettowachstum" (sales_signed, cancellations, Differenz), danach die alten vier. setRowHeights auf 3*BAND je Block.
...
- Title "Lern weiter, / auch zu Hause." approved by Ruben 18.09.2026. Lead sentence and EN texts were proposed by Claude, check with Ruben before changing.

## Blog from the Google Sheet "IMPACT Blog" (18.09.2026, Ruben: "similar to the event planner sheet")
- Sheet "IMPACT Blog" (ID 13PcwnjnrlX6kLLfrVWk3-sQTB9O7Z6aXD-_wBmUVQ7k, Drive folder "IMPACT Website" in Ruben's My Drive, own file because Google shares per file; sharing is up to Ruben). Tab "Posts", one row = one post: Website (tick = online) | Date | Title | Description (meta description) | Photo URL (Drive link or https) | Text | Website URL (link to the post, written by the script for ticked complete rows; added 18.09. on Ruben's request) | Slug (auto, never change; HIDDEN helper column since 18.09.) | Check (written by the script: OK / Missing: ...). Header notes explain every column.
- Text format: empty line = paragraph, line break = `<br>`, `# ` = subheading (h3), `- ` = list, `**bold**`, `*italic*`, `[text](url)`, bare URLs become links. Everything else is escaped.
- Apps Script (webapp version 35): `blogRead()` via `what=blog` (only ticked rows with Date/Title/Text; writes Slug and Check), `blogSetup()` formats an empty file (locale de_CH), `blogSeed()` filled the six old posts once (Script Property `blogSeeded`; checkbox cells count as content, so getLastRow is useless in that sheet).
- Cloudflare `functions/api/blog.js` (10 min cache, `?refresh=1`). GitHub workflow `.github/workflows/blog-sync.yml` daily 04:50 UTC + manual "Run workflow": `tools/blog_from_api.py` (writes data/blog.json, downloads photos once to assets/blog/<slug>.jpg via /api/event-image, resized to 1600 px with Pillow) -> `tools/build_blog.py` (article pages, card list in articles/index.html, sitemap.xml; idempotent). An existing article page is its own template (sitewide edits survive), new posts are cloned from the first article folder. Unticked row = page is removed at the next build. Safety: empty/unreadable sheet = abort, more than half of the articles disappearing = abort.
- The six Webflow posts were converted to the text format; visible text is identical. ⚠️ Dates: article pages showed "May 29, 2026" on three posts (Webflow artefact), the index showed 10 Apr / 13 May / 29 May / 25 Jun; the sheet now holds the index dates and the pages follow (are-martial-arts 10.04., zero-ego 13.05., socializing 25.06.). Descriptions are empty (old placeholder "<Title> – Artikel von IMPACT Martial Arts." stays until Ruben writes them).
- The first fetch from a GitHub runner took 2.5 min (retries), like plan_from_api. Never edit article pages' title/date/photo/body by hand: the next build overwrites them from the sheet.

## "Publish now" in the content sheets (21.09.2026, Ruben: "jetzt veröffentlichen button please")
- Blog sheet layout since 21.09.: row 1 = publish row (A1 tick box, B1 label, D1 status, G1 "Last read by the website"), row 2 = headers (`BLOG_HR = 2`), posts from row 3. `blogSheet()` migrates/repairs on its own.
- `publishOnEdit` (installable onEdit trigger, installed via `/api/blog?what=pubtrigger`, handler checks `PUB_TARGETS[sheetId]`) dispatches the GitHub workflow (`blog-sync.yml`), 2-minute guard via Script Property `pubLast:<workflow>`. Token ONLY in Script Properties `GITHUB_TOKEN` (Ruben creates and pastes it himself; Claude never sees it). Tested 21.09. 10:20 without token: box unticks, status "Not connected yet". Webapp version 37.
...
## Legal pages with header + footer (23.09.2026, Sam's request via Ruben)
- /agb/, /agb-kindertraining/, /datenschutz/, /en/terms/, /en/privacy/, /en/terms-kids/ now sit on the about-page shell (navwrap + topbar with DE/EN toggle to the exact twin, mobmenu, full footer with the legal links of that language, trbios = {} so the modal script stays quiet). Legal text `<main class="legal">` unchanged byte for byte; own CSS block `/*legal*/`. Before: logo-only nav and a one-line fbottom that linked the EN pages to the GERMAN AGB/Datenschutz. Bonus fix: h1 `overflow-wrap:anywhere` (DATENSCHUTZERKLAERUNG overflowed on 390 px, also live before).
- Lesson: replacing `href="/en/about/"` page-wide also hit the mobmenu "About us" link; toggle links must be replaced inside the `.langtog` spans only (DE toggles end with `</a></span>`, EN toggles with `</span></span>`).

## Blog: 48 Webflow posts restored (25.09.2026, Ruben: "hol die 48 Artikel ins Blog-Sheet")
- GA4 27.06.-24.09.: old Webflow blog 2'364 views / 1'416 users (top: nutrition-for-mma 300 users, fueling-up 170, self-defense mindset 115; 0 s engagement on those = maybe bots), new /articles/ only 337 views / 210 users since launch. Cause: only 6 of 54 live Webflow posts were migrated on 28.08.; the rest redirected to the index.
- Source: `tools/backup/webflow-cms/cms-teil1.json` part 6 (`blog-posts`, 57 items, 54 live; fields blogs-title, post-body HTML, post-excerpt, post-main-image CDN url, language ids fb0e=DE / f1d6=EN, createdOn used as date). Converted to the sheet text format (h3/h4 -> "# ", ul/ol -> "- ", strong -> **, em -> *, a -> [..](..), blockquote -> paragraph; ol numbering lost) into `data/blog-import.json`; photos pulled from the Webflow CDN (still online) to `assets/blog/<slug>.jpg` (1600 px, 12 MB total).
- Apps Script `blogImport()` (`what=blogimport`, passthrough in functions/api/blog.js) appends rows for slugs missing in the sheet, ticked, date dd.MM.yyyy. Webapp version 42. Result: 48 added, sheet now 54 posts; first read after the import gave a 502 (Check/URL writes for 48 rows), second read 26 s ok.
- Pages built locally and pushed (54 articles, sitemap 54). Old `/blog/<slug>` URLs resolve through the SLUG rule to the new pages (verified 3), incl. DE slugs. The DE posts sit on the same DE article template; EN posts too (as before). Popup decision: no slide-in for now (Ruben's newsletter thread), revisit when article traffic is back.

## Schedule page redesign (25.09.2026, Ruben's five points, all four schedule pages DE/EN)
- Colours = category fills read from the ZH slide SVG (exact hex): striking (Boxen, Muay Thai, Striking) #6d7b84, grappling (BJJ, Wrestling) #61a98d, Fitness Kickboxing #893c68, MMA #2874ba, Street Defense #3b5e7f, Little Ninjas #e1ab1f, Open Mat #d9d9d9 (dark text for readability, the slide uses white). Cards are filled with white text like on the slide; old per-discipline border colours are gone. Block `/*schedcolors*/` rewritten, rules prefixed `.spgrid` to beat the filter states (`big`/`big2` now = gold ring via box-shadow).
- Alternating day shading: `.daybg` divs appended after each render (absolute, measured from the `.dh` headers, odd days only, skipped when one day is shown). Kicker "STUNDENPLAN ZÜRICH" removed. `.spgrid{padding-top:30px}`, `.daytabs{padding-bottom:14px}`.
- Filter: label "Filter"/"Filters" (`.spfl`), each button wrapped in `.spfd` with its panel as a dropdown under the button (caret, close on outside click / Escape, only one open), options as a vertical list with radio dots (level) / squares (martial art). Block `/*spfilter2*/` in head; old `/*spfilter*/` CSS stays as base. On phones the second dropdown aligns right.

## exercise.com tags cleaned up (25.09.2026, Ruben)
- `functions/api/lead.js` no longer sets a daily `repeat-lead-YYYY-MM-DD` tag on repeat requests (it created one new tag per day). The date stays in the client note "ERNEUTE ANFRAGE <date>" and in the Leads Log sheet. Discipline and location tags are still added.
- Deleted in exercise.com > Tags: all 20 `repeat-lead-2026-09-xx` tags and `API test`. `start-lp` stays (Ruben: marker for website leads).
- Never delete: Migrating, Bexio (LTV / migration detection in klassen.js), Trial Zurich / Trial Winterthur (analytics), location and discipline tags. Rule (Ruben): only tags the team sets itself; no tags with dates.
- Duplicate spellings (25.09.2026, Ruben: "nur wenn es keine Kunden und Automations beeinflusst"): `lead.js` now maps form values to the existing English tag names (`TAG_MAP`/`tagOf`: Zürich->Zurich, Boxen->Boxing, Ringen->Wrestling, Thai-Boxen / Muay Thai->Thai Boxing / Muay Thai, Fitnessboxen / Kickboxen + Fitness Kickboxen->Fitness Boxing/ Kickboxing, BJJ->Brazilian Jiu-jitsu / BJJ). Only tags; "Interested in", note and sheet log keep the form value. exercise.com automations checked: "tag added: Zürich" and "tag added: Zurich" both assign Abdi, "Winterthur" assigns Bogdan, Trial Zurich/Winterthur send the trial mail + stage, "inactive" removes groups; discipline tags trigger nothing. All 278 clients with only "Zürich" already have Abdi (sub_trainer_id 2745031), so adding "Zurich" changes nothing. Re-tagging the existing clients (add English tag, then delete the German tag on the Tags page) was blocked for Claude by the safety check; Ruben does it by hand. MMA Street Defense (56) vs Street Defense (40) left alone (may be a different class, Ruben to decide).

## Newsletter sign-up on the website (25.09.2026, Ruben: 1 = tick box in the trial form, 2 = exit pop-up)
- Texts approved by Ruben: tick box "Schick mir News und Events von IMPACT (optional)", pop-up "Noch nicht bereit fürs Probetraining?" / "Wir laden dich zu unseren Events ein und halten dich auf dem Laufenden." EN versions, button "Anmelden", placeholder, fine print and success/error texts are Claude's proposal.
- Rules (Ruben): tick box never pre-ticked, visibly optional (dark box, grey text, under the submit button). Pop-up never before 30 s on the current page (also on desktop), never while a trial form is visible (even untouched), never after a form was started, not on /probetraining/, /en/trial/, thanks, /termin/, booking, cancellation, training-plan; never after a trial request; once in 30 days. Page-to-page browsing never triggers it (desktop = mouse leaves the window at the top; phone = fast scroll up). Course pages incl. ad traffic are included.
- Code: `functions/api/newsletter.js` (`subscribe()` shared with lead.js, MD5 member hash, double opt-in, tags, merge field "Location" looked up by name), `assets/nl.js`, loader + switch `NL_ON` in `assets/track.js` (false until `MAILCHIMP_API_KEY` is set and the privacy text is updated), `?nltest=1` loads it for tests. 36 trial forms carry `<label class="nlopt" hidden>`; `tools/archive/templates` not touched.
- Tested headless with a simulated clock (18 checks: 30 s, form visible, form started, phone scroll, thank-you flag, once only, switched off, EN, Escape) plus screenshots. Live 25.09. with NL_ON=false, `/api/newsletter` GET = {"configured":false}.
- SWITCHED ON 25.09.2026 evening: MAILCHIMP_API_KEY set by Ruben (secret), connection check `/api/newsletter?check=1` = 200, test sign-up ruben@ = pending. Mailchimp opt-in confirmation mail in German du form (subject "Bitte bestätige deine Anmeldung", heading, button "Anmeldung bestätigen", footer lines via Form builder > Translate it). EN visitors also get the German confirmation mail.
